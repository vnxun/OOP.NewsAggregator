...
public class the_blockchainCrawl implements Runnable {
    private static final int MAX_DEPTH = 3;
    private static final Logger logger = Logger.getLogger(the_blockchainCrawl.class.getName());
    ...
    public the_blockchainCrawl(String link) {
    ...
    private String extractArticleTitle(Document doc) {
        ...
        return titleElement != null ? titleElement.attr("content") : "Title not found";
    }

    private String extractArticleDate(Document doc) {
        Element dateElement = doc.selectFirst("meta[property=og:article:published_time]");
        if (dateElement != null) {
            String dateTime = dateElement.attr("content");
    ...
    private String extractArticleContent(Document doc) {
        Elements contentElements = doc.select("p");
    ...
    private boolean isBlockchainRelated(Document doc) {
        String paragraph = extractArticleContent(doc);
        return countBlockchainOccurrences(paragraph) >= 3; 
    }

    private String extractArticleCategory(Document doc) {
        Element categoryElement = doc.selectFirst("li.entry-category a");
    ...
    private String extractKeywords(Document doc) {
        Element keywordsElement = doc.selectFirst("meta[property=og:site_name]");
